...
def term_freevars(tm0):
    ...
    if tm0.ctag == "TMlam":
        x01 = tm0.arg1
        body_fvs = term_freevars(tm0.arg2)
        return body_fvs - {x01}
    if tm0.ctag == "TMapp":
        fv1 = term_freevars(tm0.arg1)
        fv2 = term_freevars(tm0.arg2)
        return fv1 | fv2
    raise TypeError(tm0)
#
############################################################
#
def term_freshvar(avoid_set):
    i = 0
    while True:
        candidate = f"x{i}"
...
def term_gsubst(tm0, x00, sub):
    """
    Points: 20
    This function implements the (general) substitution function on terms
    that should correctly handle an open [sub] (that is, [sub] containing 
    free variables)
    You can use the function [term_freevars] implemented in Assign01.
    """
    if tm0.ctag == "TMvar":
        x01 = tm0.arg1
        return sub if (x00 == x01) else tm0
    ...
    if tm0.ctag == "TMapp":
        tm1_new = term_gsubst(tm0.arg1, x00, sub)
        tm2_new = term_gsubst(tm0.arg2, x00, sub)
        return TMapp(tm1_new, tm2_new)
    ...
    if tm0.ctag == "TMlam":
        ...
        sub_fvs = term_freevars(sub)
        
        if x01 in sub_fvs:
            body_fvs = term_freevars(body)
            avoid = sub_fvs | body_fvs | {x00, x01}
            fresh = term_freshvar(avoid)
            renamed_body = term_subst0(body, x01, TMvar(fresh))
            new_body = term_gsubst(renamed_body, x00, sub)
            return TMlam(fresh, new_body)
        else:
            new_body = term_gsubst(body, x00, sub)
            return TMlam(x01, new_body)
